...
// Usage : npx ts-node src/scripts/simulateGen3Zones.ts [total=1000] [guildId=simulation]
...
function runZoneSimulation(zoneId: string, total: number, guildId: string): Omit<ZoneSummary, "zoneId" | "label" | "available"> {
  ...
  execSync(`npx ts-node src/scripts/simulateCapture.ts ${zoneId} ${total} ${guildId}`, {
    cwd: PROJECT_ROOT,
    stdio: "inherit",
  });
