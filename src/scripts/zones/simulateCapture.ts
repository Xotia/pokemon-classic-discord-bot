...
// Usage : npx ts-node src/scripts/zones/simulateCapture.ts <zoneId> [total=1000] [guildId]
...
async function main() {
  ...
  if (!zoneId) {
    console.error('❌ Usage : npx ts-node src/scripts/zones/simulateCapture.ts <zoneId> [total=1000] [guildId]');
